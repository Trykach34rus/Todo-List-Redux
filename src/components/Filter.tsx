...
export default function Filter() {
	...
	function handleInputChange(event: React.SyntheticEvent) {
		...
		dispatch(handleSearch(input.value))
	}

	function handleFilterChange(event: React.SyntheticEvent) {
		const input = event.target as HTMLSelectElement
		dispatch(setFilter(input.value as FilterT))
	}

	return (
		<div className='flex items-center gap-4'>
			<div className='flex-1 flex items-center gap-2 px-4 py-2 rounded border border-purple-500 focus-within:ring-2 focus-within:ring-purple-500/20 transition-all duration-200'>
				<input
					type='text'
					...
					value={inputValue}
					className='flex-1 bg-transparent text-gray-900 dark:text-white text-base font-medium outline-none placeholder:text-gray-400 dark:placeholder:text-gray-500'
				/>
				<svg
					width='21'
					height='22'
			...
			<select
				...
				className='px-3 py-2 rounded bg-purple-600 text-white font-medium uppercase outline-none focus:ring-2 focus:ring-purple-400 transition-all duration-200 cursor-pointer'
			>
			...
			<button
				...
				className='min-w-[38px] min-h-[38px] rounded bg-purple-600 flex items-center justify-center hover:bg-purple-700 transition-all duration-200'
			>
				<img
